...
async def get_images_command(message: Message):
    try:
        ...
        await bot.reply_to(message, "Silakan kirim file Excel (.xlsx, .xls, .xlsm) yang berisi gambar.")
...
async def excel_get(message: Message):
    try:
        if not message.document.file_name.endswith((".xlsx", ".xls", ".xlsm")):
            return await bot.send_message(message.chat.id, "Kirim file Excel yang valid (.xlsx, .xls, .xlsm).")
