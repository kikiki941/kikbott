...
async def handle_done(message: Message):
    try:
        async with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            await bot.send_message(message.chat.id, 'Silakan masukkan nama file TXT yang akan dihasilkan (tanpa ekstensi):')
...
async def handle_txt_name(message: Message):
    try:
        async with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            # Ambil nama file dari pesan pengguna
            output_filename = f"files/{message.text}.txt"
            data['name'] = output_filename
            
            # Gabungkan file .txt
            gabung_txt(data['files'], output_filename)

            # Hapus file-file yang telah digabungkan
